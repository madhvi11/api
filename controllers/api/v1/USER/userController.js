...
module.exports.registration=async(req,res)=>{
    ...
    try
    {
        ...
        if(checkmail)
        {
            return res.status(400).json({msg:'this email is alredy registered',staus:0})
        }
        else
        {
            if(req.body.password==req.body.confirm_password)
            {
                ...
                {
                    return res.status(400).json({msg:'register failed',status:0});
                }
            }
            else
            {
...
module.exports.editprofile=async(req,res)=>{
    ...
    try
    {
        ...
        if(req.file)
        {
            ...
            {
                return res.status(400).json({msg:'record not edit',status:0})
            }
        }
        else
        {
